...
// get all farmer
export const getAllFarmer = ( expertCode, fullName, phone, province, city, pid, marked) =>
  API.get(
...
// get detail farmer
export const getDetailFarmer = (farmerCode) => API.get(`http://panel.agroiranexpert.com/api/farmer/data?farmerCode=${farmerCode}`);

// assign farmer to expert
export const addAssignFarmer = (data) => API.post("/expert/farmer", data);

// get province
export const getProvince = () => axios.get("https://agrodayan.ir/api/provinces");
...
// search farmer
export const searchFarmers = (fullName) => API.get(`/expert/get/farmers?expertCode=ESEHO907951&fullName=${fullName}`);

// add comment
export const addComment = (data, farmerCode) => API.put(`/expert/farmer?farmerCode=${farmerCode}`, data);

// allahdadi
// add star to farmer
export const addStar = (data , farmerCode) => API.put(`/expert/farmer?farmerCode=${farmerCode}`, data)

// get questionnaire more
export const getMoreQuestionnaire = (farmerCode) => API.get(`/farmer/data/questionnaires?farmerCode=${farmerCode}`);
  // API.get(`/farmer/data/questionnaires/features?farmerCode=${farmerCode}`);
  

// get mealplan more
export const getMoreMealplan = (farmerCode) => API.get(`/farmer/data/mealPlans/features?farmerCode=${farmerCode}`);

// get visit more
export const getMoreVisit = (farmerCode) => API.get(`/farmer/data/visits/features?farmerCode=${farmerCode}`);

// export const sort farmer
export const sortAllFarmer = (expertCode, sortMeal, sortArea) => 
  API.get(`http://185.81.99.8:88/api/expert/get/farmers?expertCode=${expertCode}&mealPlan=${sortMeal ? sortMeal : ""}&area=${sortArea ? sortArea : ""}`);

export const sendIfno = (information) => API.post("/expert/carriers", information);
...
// get info expert
export const getInfoExpert = (expertCode) => API.get(`/expert?expertCode=${expertCode}`);
...
// update carrier
export const updateCarrier = (carrierData, sid) => API.put(`/expert/carriers?sid=${sid}`, carrierData);
